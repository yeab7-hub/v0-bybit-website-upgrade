...
export function PriceChart() {
  ...
  const [candles] = useState(() => generateCandles(80))
  ...
  const drawChart = useCallback(() => {
    ...
    for (let i = 0; i < candles.length; i++) {
      ...
      const highY =
        padding.top + ((maxPrice - c.high) / totalRange) * chartH
      const lowY =
        padding.top + ((maxPrice - c.low) / totalRange) * chartH
      const openY =
        padding.top + ((maxPrice - c.open) / totalRange) * chartH
      const closeY =
        padding.top + ((maxPrice - c.close) / totalRange) * chartH
    ...
    if (mousePos) {
      ...
      if (
        ...
        mousePos.y <= padding.top + chartH
      ) {
        const price =
          maxPrice -
          ((mousePos.y - padding.top) / chartH) * totalRange
    ...
    const lastY =
      padding.top + ((maxPrice - lastCandle.close) / totalRange) * chartH
    const isLastGreen = lastCandle.close >= lastCandle.open
    ctx.strokeStyle = isLastGreen
      ? "hsl(142, 72%, 50%)"
      : "hsl(0, 72%, 51%)"
    ctx.lineWidth = 0.5
    ctx.setLineDash([2, 3])
  ...
  const isGreen = displayCandle.close >= displayCandle.open
  const changePercent =
        ...
        <div className="flex items-center gap-6">
          ...
          <div className="hidden items-center gap-6 lg:flex">
            ...
            <div>
              ...
              <div className="font-mono text-xs text-foreground">12,438.52</div>
            </div>
            <div>
              <div className="text-[10px] text-muted-foreground">24h Vol(USDT)</div>
              <div className="font-mono text-xs text-foreground">1.21B</div>
            </div>
          </div>
        </div>
