...
export function OrderForm({ pair = "BTC/USDT" }: { pair?: string }) {
  ...
  const handlePercentClick = useCallback((pct: number) => {
    ...
    if (side === "buy" && effectivePrice > 0) {
      const maxAmount = (quoteBalance * pct / 100) / effectivePrice
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        // Refresh balances, orders, and trades
      ...
      <div className="flex border-b border-border">
        <button
          ...
          className={`flex-1 py-2.5 text-center text-sm font-semibold transition-colors ${
            isBuy ? "border-b-2 border-success bg-success/5 text-success" : "text-muted-foreground hover:text-foreground"
          }`}
        >
          Buy
        </button>
        <button
          ...
          className={`flex-1 py-2.5 text-center text-sm font-semibold transition-colors ${
            !isBuy ? "border-b-2 border-destructive bg-destructive/5 text-destructive" : "text-muted-foreground hover:text-foreground"
          }`}
        >
          Sell
        </button>
      ...
      <div className="flex-1 overflow-y-auto p-3">
          ...
          {(["Limit", "Market", "Stop-Limit"] as const).map((type) => (
            <button
              ...
              className={`rounded-md px-2.5 py-1 text-xs font-medium transition-colors ${
                orderType === type ? "bg-secondary text-foreground" : "text-muted-foreground hover:text-foreground"
              }`}
            >
              {type}
            </button>
        ...
        <div className="mb-4 flex items-center justify-between">
          ...
          <span className="font-mono text-xs text-foreground">
            {isBuy
              ? `${Number(quoteBalance).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })} ${quoteAsset}`
        ...
        {orderType === "Stop-Limit" && (
          <div className="mb-3">
            ...
            <div className="flex items-center rounded-md border border-border bg-secondary/30 px-3 py-2">
              <input type="text" value={stopPrice} onChange={(e) => setStopPrice(e.target.value)} placeholder="Stop Price" className="flex-1 bg-transparent font-mono text-sm text-foreground outline-none placeholder:text-muted-foreground" />
        ...
        {orderType !== "Market" ? (
          ...
          <div className="mb-3 rounded-md border border-border bg-secondary/20 px-3 py-2.5 flex items-center justify-between">
        ...
        <div className="mb-4 flex items-center gap-2">
          {percentages.map((pct) => (
            <button key={pct} onClick={() => handlePercentClick(pct)} className={`flex-1 rounded-md py-1 text-[10px] font-medium transition-colors ${selectedPercent === pct ? (isBuy ? "bg-success/20 text-success" : "bg-destructive/20 text-destructive") : "bg-secondary text-muted-foreground hover:text-foreground"}`}>
              {pct}%
            </button>
          ))}
        </div>
        ...
        {/* Feedback */}
        {feedback && (
          <div className={`mb-3 rounded-md px-3 py-2 text-xs font-medium ${feedback.type === "success" ? "bg-success/10 text-success" : "bg-destructive/10 text-destructive"}`}>
            {feedback.msg}
          </div>
        )}
        ...
        <Button
          ...
          className={`w-full py-3 text-sm font-semibold ${
            isBuy ? "bg-success text-success-foreground hover:bg-success/90" : "bg-destructive text-destructive-foreground hover:bg-destructive/90"
          }`}
        >
